...
public class WCS extends Converter {
    ...
    /**
     * This includes a nominal 'scale' for the WCS. While this can often be
     * calculated from the transformation, that may sometimes be difficult.
     */
    ...
    /**
     * Return a new WCS from the existing WCS where we add a scale. This allows
     * us to return a WCS for a subset of an image easily.
     */
    ...
    /**
     * Find which axes are being used for coordinates. Normally this will just
     * be 1 and 2, but occasionally we may be surprised.
     */
    ...
    /** The GLS projection is an SFL projection where we
     *  give the CRVALs relative to the center of the actual
     *  image rather than using 0,0.  It is not an oblique SFL
     *  projection, but a normal SFL projection with
     *  an added translation in the projection plane.
     *  To fix this we reset the CRVAL and CRPIX parameters 
     *  to the values appropriate for the corresponding SFL projection.
     */
    private void convertGLSToSFL() {
        double crp1 = h.getDoubleValue("CRPIX1");
        double crp2 = h.getDoubleValue("CRPIX2");
        ...
        // Find the scaling matrix, CD
        if (h.containsKey("CDELT1")) {
            cd11 = h.getDoubleValue("CDELT1");
            cd22 = h.getDoubleValue("CDELT2");            
        } else {
            if (!h.containsKey("CD1_1")) {
                throw new IllegalArgumentException("No CD data for GLS to SFL translation");
            }
            cd11 = h.getDoubleValue("CD1_1");
            cd12 = h.getDoubleValue("CD1_2");
            cd21 = h.getDoubleValue("CD2_1");
            cd22 = h.getDoubleValue("CD2_2");
        }
        ...
        // Haven't thought about how to calculate proper offset
        // when non-diagonal terms are non-zero.
        if (cd12 != 0 || cd21 != 0) {
            throw new IllegalArgumentException("GLS-SFL with non-zero rotation terms");
        }
 
        
        // We're assuming lon is first coordinate.
        // Don't go too far to get to 0 longitude.
        if (crval1 > 180) {
            crval1 -= 360;
        }
        if (crval1 < -180) {
        ...
        // What offset should the reference pixel be at?
        double off2 = -crval2/cd22;
        double off1 = -crval1/cd11*cos(toRadians(crval2));
        
        
        // OK.  Now we add this to the CRPIXs to move the origin.
        try {
            h.addValue("CRPIX1", off1+crp1, "");
            h.addValue("CRPIX2", off2+crp2, "");
            h.addValue("CRVAL1", 0., "");
            h.addValue("CRVAL2", 0., "");
        } catch (HeaderCardException e) {
            throw new IllegalArgumentException("Unchangeable FITS header in GLS-SFL fix");
        }
    }
    ...
    private void extractProjection() throws TransformationException {
        ...
        if (lonType.equals("AIT")) {
            ...
            if (lonType.equals("TAN") || lonType.equals("SIN") || lonType.equals("ZEA")
                ...
                if (lonType.equals("ZPN")) {
                    
                    // Extract polynomial coefficients.
                    // We don't consider an n=0 coefficient, though
                    // that is legal.

                    for (int i = 1; i < 20; i += 1) {
                        double val = h.getDoubleValue("PV" + latAxis + "_" + i, 0);
                        if (val != 0) {
                            Settings.put("_PROJP" + i, val + "");
                        }
                    }
                }
    ...
    /**
     * Write FITS WCS keywords given key values. Only relatively simple WCSs are
     * handled here. We assume we are dealing with axes 1 and 2.
     *
     ...
     * string gives the equinox of the coordinate system. E.g., J2000, B1950,
     * Galactic, "E2000", "H2020.10375".
     */
     ...
     *
     * @param pix The pixel location we want to approximate around.
     * @return A more standard WCS with the distortion removed (locally).
     */
    public WCS approximate(double[] pix) throws TransformationException {
        CoordinateSystem cs = getCoordinateSystem();
        Projection p = getProjection();
        Scaler s = new Scaler(0, 0, 1, 0, 0, 1);
        Scaler old = getScaler();
        // The CoordinateSystem and Projection take care of
        // everything other than the Scalers and Distorters.  We're going to
        // loop over the components and look at only the Scalers
        // and Distorters.  The scalers we'll add to our base Scaler
        // and the Distorters will be replaced by a Scaler based
        // upon the Jacobian of the Distorter.

        // First we transform pix back to a unit vector...
        // We'll transform it step by step on the way back.
        double[] curr = inverse().transform(pix);
        for (Transformer t : getComponents()) {
            if (t instanceof Distorter) {
                double[][] mat = ((Distorter) t).jacobian(curr);
                double[] next = t.transform(curr);

                // We approximate the distorter by a scaler
                // that shifts the input position correctly
                // and which has a constant Jacobian equal to the correct
                // local value.
                //
                // Let's see where the matrix alone shifts us to...
                Scaler ts = new Scaler(0., 0., mat[0][0], mat[0][1], mat[1][0], mat[1][1]);
                double[] tp = ts.transform(curr);
                // Now we add in a shift so that we move to the correct location.
                double dx = next[0] - tp[0];
                double dy = next[1] - tp[1];

                s = s.add(new Scaler(dx, dy,
                        mat[0][0], mat[0][1], mat[1][0], mat[1][1]));
            } else if (t instanceof Scaler) {
                s = s.add((Scaler) t);
            }
            // Transform the point to the input for the next step.
            curr = t.transform(curr);
        }
        return new WCS(cs, p, s);
    }
}
