...
public class Imager {
	...
	System.err.println("Imager starting v"+getVersion()+".");
	...
	Image[]  cand  = surv.getImages(pos, maxSize, wcs.getScale()*180/Math.PI);
...
//	    doneImages.put(surveyID, new SoftReference(new ImageState(cand, output, match, samp, dsamp, processes)));
	...
	for(String key: keys) {
		...
		h.insertHistory(key + " is null");
                ...
                // Make sure the content is valid for the FITS header.
                val = val.replaceAll("[^\\x20-\\x7E]", " ");
                if (val.equals("1")) {
		    h.insertHistory(key);
	        } else {
