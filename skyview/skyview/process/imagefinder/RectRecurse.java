 ...
 * <dl>
 * <dt>MinEdge<dt> Only images where we are at least this number of pixels
 * from the edge will be considered.  The default is 0.
 * <dt>MaxRadius<dt> Only images where the offset of the pixel from the center of the 
 * image is less than this radius (calculations in pixels) will be considered.
 * The default is infinity.
 * <dt>CheckNaNs<dt>  is a flag whose value is ignored.  If set, then
 * images are checked to see if the data value corresponding to a 
 * pixel position is a NaN when looking for the best image.  Images
 * where a NaN is found are ignored.  Note that this applies only
 * to the boundaries of the rectangles that are checked.  If there
 * are NaNs in isolated regions or pixels these may still be used by this image
 * finder.
 * <dt>CornersOnly<dd> is a flag whose value is ignored.  If set, then
 * only the corners of rectanges are checked before the rectangle
 * is filled in with values.  This was essentially the behavior of the
 * default image finder prior to v2.02 of Skyview.
 * </dl>
 * <ul>
 *   <li> The output image is sampled in a rectangular grid with
 *        the samples spaced no more than half the image size of the
 *        input images.
 *   <li> For each rectangle, the corners of the image are projected
 *        into the frame of each currently valid image in turn.
 ...
 *        projects to pixel coordinates with x and y both less than 0).  If the
 *        other three corners are also below the image, even if some are to the
 *        right of the image or some have X-values that would be within the image,
 *        then the image can be eliminated from future consideration.  However, if
 *        even one of the other pixels is not below the image, then we can not
 *        use this criterion to eliminate the image, even if that corner is also
 *        outside the image.  Similarly if the other three corners all project
 *        to the left of the image, then the image can be eliminated.
 * 
 *   <li> The best pixel is found for each corner.  The algorithm for the
 *        best pixel is defined by subclasses.
 *        If the pixel does not fall within any image it is marked as NO_COVERAGE.
 *   <li> A pixel may not correspond to any position on the sky (e.g., a pixel outside
 *        the elliptical bounds the Aitoff projection).  These pixels are marked
 *        as NON-PHYSICAL
 *   <li> If the best images are the same for all four corners, then the edges of the
 *        rectangle is also checked.  If the entire border comes from the same candidate
 *        then the rectangle is filled in as coming from that sample.
 *   <li> Otherwise the algorithm checks to see if the the values along the horizontal
 *        or veritical edges are the same.  If so, we split the rectangle in half, so
 *        that the corners that match are kept together.  If not we do a recusion
 *        splitting the rectangle into quarters.
 *  </ul>
 */
public abstract class RectRecurse extends ImageFinder {
	...
	if (count > 0) {
		...
		System.err.println("  "+count+" of "+input.length+" candidates selected.");
	    }
	    return img;
	} else {
	    return null;
     ...
     *  @param std   The value each edge is to be compared against.
     ...
     *  @param newValid  If we recurse should be consider this image?
	...
	for (int i=0; i<input.length; i += 1) {
	    ...
	    for (int j=0; j<corners.length; j += 1) {
	        t2 = getImage(input[i], getCelest(corners[j]));
    ...
    protected int[] getInputLimits(Image in) {
        return new int[]{in.getWidth(), in.getHeight()};
    }
	...
	t3 = getCelest(pix);
	double[] tzz = skyview.geometry.Util.coord(t3);
	...
	for (int i=0; i< input.length; i += 1) {
	    ...
	    if (secondTry || valid[i]) {
	        ...
	        double ty = t2[1];
                ...
                int[] limits = getInputLimits(input[i]);
	        ...
	        if (mn >= edgeMin) {
		    if (radiusCheck) {
			double radsq = radiusSquared(tx,ty, nx,ny, i, input[i]);
			if (radsq > radMaxSq) {
			    continue;
			}
		    }		
     ...
     *  The last two arguments may be used in extending classes.
     *  when the center of the image
     *  is not the center of the field of view.
     */
    protected double radiusSquared(double tx, double ty, double nx, double ny, 
				   int index, Image input) {
	return  (tx-0.5*nx)*(tx-0.5*nx) + (ty-0.5*ny)*(ty-0.5*ny) ;
    }
	
    
    /** The criterion function should return a larger value
     *  for more desirable inputs.
     *  @param image  The index of the image being considered
     *  @param nx,ny  The size of the candidate image
     *  @param tx,ty  Where the desired pixel would be sampled from the
     *                candidate.
     */
    protected abstract double criterion(int image, int nx, int ny,
					double tx, double ty);
