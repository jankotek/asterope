 ...
 * differences.
 *
 * Given a sequence of integers we: (** for steps where we actually write
 * something) <ol> <li> ** Write out the value of the first integer <li> Replace
 * the sequence with the sequence of differences. <li> Replace positive
 * differences, n, with 2*n and negative differences with -2*n-1. So 0 -> 0, -1
 * -> 1, 1 -> 2, -2 -> 3, 2 -> 4,... This means that differences will have the
 * same number of bits (pretty much) for a given absolute value. <li> Loop over
 * the sequence in blocks of 16 or 32 integers. There may be a shorter terminal
 * block. <li> For each block: <ol> <li> Calculate the sum of the values and get
 * the average of this sum. Find the number of bits it takes to represent this
 * value, e.g., if the average is 110.4 then we would need 7 bits to represent
 * this (2^7 = 128 which is > 110.4). Call this number F. <li> ** Write out
 * (F+1) (using a number of bits appropriate for the data size). We use F+1
 * rather than F to cater to the first special case below. <li> For each value
 * in the block <ol> <li> Break up the number into two parts: the top,T, any
 * bits more significant than the F low order bits, and the bottom, B, the
 * lowest F bits in the value. To get the top we can divide by 2^F or left shift
 * F bits. To get the bottom we can do a modulo operation or a bitwise AND with
 * the mask 2^F - 1. <p> The top will often be 0 and will always be less than or
 * equal to the number of values in the block. [In the worst case only one
 * difference in the block in non-zero, then the average difference is the just
 * that difference over the block size.]. Typically it will be 0, 1, or 2. <li>
 * ** Write out T 0 bits followed by a single 1 bit. <li> ** Write out the B
 * using just F bits. <li> In a typical case we will write about B+2 bits for
 * each value. Say we have 16 bit pixels with values fluctuating typically by 30
 * units between adjacent pixels. Then we will write ~8 bits per pixel for a
 * savings of >50% in storage. Overheads include writing the first value at the
 * beginning of the file and 4 bits (for shorts) per block giving the value of F
 * for each block. If we use 32 pixel blocks, a block of uncompressed data will
 * be 512 bits while the compressed block would be ~240 bits. </ol> </ol> </ol>
 * There are two special cases for each block. <ul> <li> If all the differences
 * are 0, then instead of writing F+1 we just write 0 to mark the block as
 * constant. <li> If the differences between adjacent pixels are too high, i.e.,
 * there are largely random values in the original data, then splitting into top
 * and bottom wastes a bit of space and time. This is indicated when F is
 * greater than some maximum value. When we see this we don't do the splitting
 * we just write the values directly to the output. </ul>
 *
 * @author tmcglynn
 */
public class Rice implements CompressionScheme {
     ...
     *
     * This routine sets up input and output processing streams.
     */
     ...
     */
    public void compressStream(DataInputStream ds, OutputBitStream bo, int len)
            throws IOException {
     ...
     *
     * @param first Is this the first block in the compression stream?
     * @param lastWord What was the last word (not difference) read int from the
     * input. If this is the first block it should be the first word in the
     * file.
     * @param len The number of words in this block
     * @param ds The input stream from which we are reading data.
     * @param bo The output bit stream on which we are writing results.
     * @return The last word read (not the last difference).
     */
    private int compressBlock(boolean first, int lastWord, int len,
            DataInputStream ds, OutputBitStream bo)
            throws IOException {
     ...
     *
     * @param val The input value to be emitted (normally a pixel difference).
     * @param fs The number of 'noise' bits for this block.
     * @param bo The output bit stream.
     * @param mask A mask getting the noise bits (= (1<<fs)-1).
     */
     ...
     * possible
     */
    private void specialCaseRandom(OutputBitStream bo, int[] diffs)
            throws IOException {
     ...
     *
     * @param lastWord The last word in the previous output. For the first call
     * this should be the same as the first input element of diffs. For all
     * later calls it should be the last input value of diffs in the previous
     * call.
     * @param diffs On input this contains the original integer values. On
     * output it contains the integer differences coded so that all values are
     * positive. Positive values go to 2*val Negative values got to 2*|val|-1.
     ...
     * @param len The number of pixels expected on the input.
     * @return A byte array representing the uncompressed data. This may need to
     * be read through a ByteArrayInputStream to recover the appropriate pixel
     * values (which may not be bytes).
     */
    ...
    public void decompressStream(InputBitStream bin, DataOutputStream ds, int len)
            throws IOException {
    ...
    private void decompressBlock(int word, InputBitStream bin, DataOutputStream ds, int len)
            throws IOException {
    ...
    /** Write a block of 'random' values, i.e., where the noise
     *  is to large to be usefully compressed.
     */
    private int decompressRandom(int word, InputBitStream bin, DataOutputStream ds, int len)
            throws IOException {
        for (int i = 0; i < len; i += 1) {
