...
/** 
 * This class implements the H compression and decompression algorithm.
 * 
 * This compression scheme works as follows:
 * Suppose we have an input array a[n,m]
 * 
 *  <ul>
 *    <li> ** Write a magic number, followed by the dimensions of the 2-d image
 *          and the scale value.
 *    <li> Create a stream of the sign bits of each non-zero element in the array.
 *         (no sign bits are saved for zeroes).  Save this array.  Replace each
 *         negative value in the in the array with it's absolute value.
 *    <li> Calculate the maximum number of bits needed in each quadrant of the image.
 *         For reasons that are unclear to me two of the quadrants are combined
 *    <li> ** Write out the three values for the number of bits in the quadrants. 
 *    <li> Starting with a stride of one replace the four values
 *         a[n,m], a[n+stride,m], a[n,m+stride], a[n+stride,m+stride] with
 *         the sum of the four numbers and three differences (with two
 *         positive and negative elements) over the array.  I.e., first do
 *         a[00],a[01],a[10],a[11], then process a[02],a[03],a[12],a[13], then...
 *         until the entire array has been used.  Then double the stride so that
 *         we we are just processing the elements where we put in the sum in
 *         the first iteration, a[00], a[02], a[20], a[22], and keep going
 *         until we have processed the entire image up to the maximum power of two
 *         less than the dimension of the image.  Eventually a[00] will have
 *         the sum of all pixels in the image.
 *    <li> Rearrange the pixels in the image, so that the pixels that are were
 *         sums the longest in the process come first.  
 *    <li> Apply any digitization of the image.  This substantially enhances compression
 *         but makes the compression lossy.
 *    <li> For each bit-plane in the image extract the bits in sets of four, so that
 *         we get a nybble.  Apply a Huffman compression to these nybbles.  For
 *         reasons that are unclear this is step done separately for each quadrant.
 ...
 * 
 * 
 * The original hcompress code written by Rick White (USRA/STScI) and
 * this code is based upon that but has been extensively modified in the transition from C.
 * The original code includes the following copyright. 
 * 
 * ** Copyright (c) 1993 Association of Universities for Research   **
 * ** in Astronomy. All rights reserved. Produced under National    **
 * ** Aeronautics and Space Administration Contract No. NAS5-26555. **
 * 
 * The bit twiddling functions found in the original HCompress library are largely
 * handled within the In/OutputBitStream methods which provide more general (and
 * presumably a bit less efficient) support for consuming or emitting a bit stream.
 * 
 */
public class HCompress implements CompressionScheme {
     ...
     */
    static final int[] code_magic = {0xDD, 0x99};
    /*
    ...
    public void initialize(Map<String, String> params) {
        try {
            ...
            scale = Integer.parseInt(params.get("scale"));
        } catch (Exception e) {
            throw new IllegalArgumentException("Required HCompress parameters not found", e);
        }
        initialized = true;
    }
    ...
    /**
     * Encode 2-D array onto bit stream.
     *
    ...
    /* ######################################################################### */
    void doencode(int a[], OutputBitStream os, byte[] nbitplanes)
            throws IOException {
    ...
    void qtreeEncode(int a[], int offset, int n, int nqx, int nqy, int nbitplanes,
            OutputBitStream os) throws IOException {
...
//        byte[] buffer  = new byte[bmax];
    ...
    /* ######################################################################### */
    /*
     * This routine does the Huffman encoding of the output bytes.
    ...
    private void writeDirect(int a[], int offset, int n,
    ...
    /** Decompress the input stream.  The result 
     * 
     * 
    ...
    public int[] decomp(byte[] input) throws IOException {
        ...
        InputBitStream dis = new InputBitStream(
                new ByteArrayInputStream(input));
     ...
     */
    private void qtreeBitInsert(int[] a, byte d[],int nx,int ny,
    ...
    private void readDirect(int[] a, int off,int n,int nqx,int nqy,
			      byte scratch[],int bit, InputBitStream dis) 
            throws IOException {
        int i;
     ...
     */
    private void qtreeExpand(int[] a, byte d[],int nx,int ny, byte b[], InputBitStream dis)
                 throws IOException {
        int i;
        ...
        for (i = nx*ny-1; i >= 0; i--) {
           if( b[i]!= 0 ) b[i] = (byte) decodeHuffman(dis);
        }
    }
     ...
     */
    private void qtreeDecode(int[] a, int off,int n,int nqx,int nqy,int nbitplanes,
            InputBitStream dis) throws IOException {
    ...
    private void dodecode(InputBitStream dis, int[] a, int[] nbitplanes)             
           throws IOException {
