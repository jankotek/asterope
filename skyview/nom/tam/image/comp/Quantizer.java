...
public class Quantizer {
    ...
    /**
     * Create a quantizer that will transform data to/from integer values using the
     * algorithm: i = round(r*scale-offset + rand - 0.5) where the rand term
    ...
    /**
     * This method is called for each tile to output the data for
     * that tile.  A byte array representing the output integer data
     * is returned.  This method calls recurseOut to descend to the
     * low level primitive arrays and which in turn calls process to
     * spit out the actual data.
     * 
     * @param input The input image, a multidimensional float or double array.
     * @param td A descriptor of the current tile.
     * @param tileIndex The tileIndex which is used to get the appropriate
     * dither values.
    ...
    /**
     * Restore a tile of the original data using the input byte array as
     * input.  The method calls recurseIn to descend to the bottom level
     * of the array which calls the input method when it reaches
     * the bottom level of the array to actually read and convert
     * the data values.
     * 
     * @param tileStream  The stream of uncompressed bytes from the tile.
     * @param tileData    The data array to be filled.
     * @param td        The corners and size of the tile.
