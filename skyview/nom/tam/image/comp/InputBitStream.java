...
/**
 * This class opens a regular input stream and allows
 * the user to make requests for 0-32 bits at a time.
 * It assumes big-endian integers as would be found in a FITS file.
 * 
 * Bits are read high-order bits first
 * @author tmcglynn
 */
public class InputBitStream {
    ...
    private int inOffset = BYTESIZE;  // Start saying we've consumed the current byte
    ...
    public int readBits(int n) throws IOException {
        ...
        while (need > 0) {
            ...
            // We want to transfer the bits from bit inOffset to inOffset+getting-1 in the
            // currentByte, to bits need-getting to need-1 in the output.
            // So 
            //    Shift the currentByte to ignore the bits we're not reading yet.
            //    Or with the mask to get only the bits that haven't already been read
    ...
    /** Skip a set of either ones or zeroes and
    ...
    public int skipBits(boolean ones) throws IOException {
        ...
        while (true) {
            ...
            int msk       = (1<<bitsLeft)-1;  // A mask of ones of the appropriate length.
