...
/**
 * This class opens a regular output stream and allows
 * the user to write 0-32 bits at a time.  It assumes big-endian integers
 * as would be used in a FITS file.
 * @author tmcglynn
 */
public class OutputBitStream {
    ...
    public void writeBits(int val, int n) throws IOException {
        ...
        while (need > 0) {
            ...
            int getting = BYTESIZE-outOffset;  // The number of bits to get on this iteration.0
            if (getting > need){
                getting = need;
            }
            ...
            // The endianess controls the location at which we extract and insert
            // bits.
            if (littleEndian) {
                base = got;
                dest = outOffset;
    ...
    /** How my bytes have been used in this stream.  This does not
     *  include the currentByte even if some bits of that have been filled.
     */
    public int size() {
        return length;
    }
